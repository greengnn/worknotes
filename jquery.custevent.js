(function($) {
	var indexOf = function(arr, obj, fromIdx) {
		var len = arr.length;
		fromIdx |= 0; //取整
		if (fromIdx < 0) {
			fromIdx += len;
	...
	var methodize = function(helper, attr) {
	...
	var CustEventTargetH = {
		...
		on: function(target, sEvent, fn) {
			var cbs = (target.__custListeners && target.__custListeners[sEvent]) || error("unknown event type", TypeError);
			if (indexOf(cbs, fn) > -1) {
				return false;
			}
			cbs.push(fn);
		...
		fire: function(target, sEvent, eventArgs) {
			if (sEvent instanceof CustEvent) {
				var custEvent = $.extend(sEvent, eventArgs);
				sEvent = sEvent.type;
			} else {
	...
	var methodized = methodize(CustEventTargetH); 
	$.extend(CustEventTarget.prototype, methodized);

	CustEvent.createEvents = function(target, types) {
		CustEventTargetH.createEvents(target, types); 
		return $.extend(target, methodized);//尊重对象本身的on。
	};
	 ...
	 */
	$.CustEvent = CustEvent;
	$.CustEventTargetH = CustEventTargetH;
	$.CustEventTarget = CustEventTarget;
